...
const ForgotPassword = () => {
  ...
  const {
    ...
    error: senderror,
    message: sendmessage,
  } = useSelector((state) => state.sendcode);

  const {
    loading: validateloading,
    error: validateerror,
    message: validatemessage,
  } = useSelector((state) => state.validatecode);
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    dispatch(findUserForgotPassword(email, setVisible, setUserInfo));
  };

  const submitHandler1 = (e) => {
    e.preventDefault();
    dispatch(sendCode(email, setVisible, setUserInfo));
  };

  const submitHandler2 = (e) => {
    e.preventDefault();
    dispatch(validateCode(email, code, setVisible, setUserInfo));
  };

  const submitHandler3 = async (e) => {
    e.preventDefault();
    await dispatch(resetPassword(email, password, confirmPassword));
    navigate("/login");
  };
  ...
  return (
    <>
      <header>
        ...
        <div className="header_right_forgot">
        ...
        {visible === 1 && userInfo && (
          <div className="login">
            <div className="loginform">
              <h2 className="forgot_head">Send Code</h2>
              <p className="para">
                Your account has been found now you click on the send button to
                receive the code.
              </p>
