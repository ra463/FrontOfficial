...
import {
  searchMissingHistory,
  searchMissingPerson,
  displayMissingSearchHistory,
  deleteMissingSearchHistory,
...
const SearchMenuFound = ({ setShowSerchMenu }) => {
  const { missing, loading: searchloading } = useSelector(
    (state) => state.missingsearch
  );
  const { displayhistory2 } = useSelector(
    (state) => state.displaymissinghistory
  );
  const { error, message } = useSelector((state) => state.removemissinghistory);
  ...
  const serchHandler = async (e) => {
    if (searchPerson === "") {
      ...
      await dispatch(searchMissingPerson(searchPerson));
      setSearchResult(missing);
    }
  };

  const addtohistory = async (id) => {
    await dispatch(searchMissingHistory(id));
  };
  ...
  const getHistory = async () => {
    await dispatch(displayMissingSearchHistory());
  };

  const removeHistory = async (id) => {
    await dispatch(deleteMissingSearchHistory(id));
    dispatch(displayMissingSearchHistory());
  };
...
export default SearchMenuFound;
